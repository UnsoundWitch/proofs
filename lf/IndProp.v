...
Proof.
  ...
  - intros H. induction H.
    ...
    + simpl. apply ev_sum. apply IHev'1. apply IHev'2.
  - intros H. induction H.
    + apply ev'_0.
    + assert (2 + n = S (S n)).
...
Proof.
  ...
  - symmetry in H0. apply Plus.plus_is_O in H0.
    destruct H0. rewrite H in H1. rewrite H in H2.
    simpl in H1. simpl in H2. apply ev_sum.
    apply H1. apply H2.
  - inversion H2.
    + symmetry in H4. apply Plus.plus_is_O in H4.
      destruct H4. rewrite H3 in H1. rewrite H4.
      simpl in H1. simpl. rewrite PeanoNat.Nat.add_0_r.
      apply H1.
    + assert (ev ((n + m) + (n + p))).
      rewrite <- H. rewrite <- H3. apply ev_sum.
      apply ev_SS. apply H0. apply ev_SS. apply H4.
      assert (n + m + (n + p) = (n + n) + (m + p)).
      rewrite plus_assoc. rewrite (plus_comm (n + m) n).
      rewrite plus_assoc. rewrite (plus_assoc (n + n) (m)).
      reflexivity.
      rewrite H6 in H5. apply (ev_ev__ev (n+n) (m+p)) in H5.
      apply H5. apply ev_same.
Qed.
(** [] *)
...
Proof.
  intros m n o H0 H1.
  induction H1. apply H0. apply le_S. apply IHle.
Qed.

Theorem O_le_n : forall n,
  0 <= n.
Proof.
  intros n. induction n. apply le_n.
...
Theorem n_le_m__Sn_le_Sm : forall n m,
  n <= m -> S n <= S m.
Proof.
  intros n m H. induction H. apply le_n.
  apply le_S. apply IHle.
Qed.

Theorem Sn_bigger : forall n m,
    S n <= m -> n <= m.
Proof.
  intros n m H. induction H. apply le_S. apply le_n.
  apply le_S. apply IHle.
Qed.
...
Theorem Sn_le_Sm__n_le_m : forall n m,
  S n <= S m -> n <= m.
Proof.
  intros n m H0. inversion H0. apply le_n.
  induction H0. apply Sn_bigger in H1. apply H1.
  apply IHle.
Qed.
...
Proof.
  intros a b.
  induction b.
  - simpl. rewrite PeanoNat.Nat.add_0_r. apply le_n.
  - simpl. rewrite <- PeanoNat.Nat.add_succ_comm.
    rewrite plus_Sn_m. apply le_S. apply IHb.
Qed.
...
Proof.
  ...
  - induction m.
    + right. apply O_le_n.
    + rewrite plus_Sn_m in H. apply Sn_bigger in H.
      apply IHn in H as H'. rewrite plus_comm in H.
      rewrite plus_Sn_m in H. rewrite plus_comm in H.
      rewrite <- plus_Sn_m in H. apply IHm in H.
...
Theorem lt_S : forall n m,
  ...
  n < S m.
Proof.
  intros n m H0. apply le_S. apply H0.
Qed.

Search (S ?n < S ?m).

Theorem lt_S_eq : forall n m,
    n < m <-> S n < S m.
Proof.
  intros n m. split.
...
Proof.
  intros n1 n2 m H0. split.
  induction n2. rewrite <- plus_n_O in H0.
  apply H0.
  apply IHn2. rewrite plus_comm in H0. apply lt_S in H0.
  rewrite plus_Sn_m in H0.
  apply Lt.lt_S_n in H0. rewrite plus_comm in H0.
  apply H0.
  induction n1. rewrite plus_comm, <- plus_n_O in H0.
  apply H0.
  apply IHn1. apply lt_S in H0. rewrite plus_Sn_m in H0.
  apply Lt.lt_S_n in H0.
  apply H0.
Qed.

Theorem longlong_eq : forall n m,
...
Proof.
  ...
  induction m.
  intros n H. induction n. apply le_n. inversion H.
  intros n H. induction n. apply O_le_n. apply n_le_m__Sn_le_Sm.
  apply IHm. rewrite <- H. rewrite longlong_eq. rewrite H.
...
Inductive prefseq : list nat -> list nat -> Prop :=
| pref_nil (l : list nat) : prefseq [] l
| pref_add (x : nat) (l1 l2 : list nat) (H : prefseq l1 l2) : prefseq (x::l1) (x::l2).
...
Proof.
  ...
  - generalize dependent s1. induction s2. 
    ...
    + intros s1 H. simpl in H. inversion H. inversion H3.
      simpl. assert (LEq: s3 = s2 -> x :: s3 = x :: s2).
      intros H'. induction s2. rewrite <- H'. reflexivity.
...
Proof.
  ...
  - intros H. destruct H. induction H.
    ...
    + simpl. rewrite andb_true_iff. split.
      apply IHexp_match1. apply IHexp_match2.
    + simpl. rewrite orb_true_iff. left.
      apply IHexp_match.
  ...
  - intros H. induction re.
    ...
    + simpl in H. rewrite orb_true_iff in H. destruct H.
      apply IHre1 in H. destruct H. exists x. apply MUnionL, H.
      apply IHre2 in H. destruct H. exists x. apply MUnionR, H.
    + exists []. apply MStar0.
Qed.
...
Lemma app_head_same: forall T (s x y: list T),
    x = y -> s ++ x = s ++ y.
Proof.
  intros T s x y H.
  induction H.
...
Proof.
  ...
  - injection Heqre'. intros H'. rewrite H' in *. apply IHexp_match2 in Heqre'.
    destruct Heqre'. exists (s1 :: x). simpl. destruct H1. split.
    apply app_head_same. apply H1. intros s'. intros. destruct H3. rewrite <- H3.
    apply H. apply H2, H3.
Qed.
