...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => 0
  | hd :: tl => if hd =? v then 1 + count v tl else count v tl
  end.
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
(*
Theorem bag_theorem : forall (n : nat), forall (b : bag), 1 + count n b = count n (add n b).
Proof.
  intros n b.
  simpl.
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l.
Proof.
  intros l. induction l as [|hd tl IHl'].
  - reflexivity.
...
Proof.
  intros l1 l2. induction l1 as [|hd tl IHl'].
...
Theorem eq_nat_same : forall n:nat,
    eq_nat n n = true.
Proof.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | nil, _ => false
  | _, nil => false
  | h1 :: t1, h2 :: t2 => if eq_nat h1 h2 then eqblist t1 t2 else false
  end.
(* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
Proof.
  intros l. induction l as [|hd tl IHl].
  - reflexivity.
  - simpl. rewrite <- IHl. rewrite -> eq_nat_same. reflexivity.
Qed.
(** [] *)
...
Theorem rev_injective : forall (l1 l2 : natlist),
    rev l1 = rev l2 -> l1 = l2.
Proof.
  intros l1 l2 IHl. induction l1 as [|hd tl IHl1].
  - simpl. rewrite <- rev_involutive. rewrite <- IHl. reflexivity.
  - simpl. rewrite <- rev_involutive. rewrite <- IHl. simpl.
    rewrite -> rev_app_distr. simpl. rewrite -> rev_involutive. reflexivity.
Qed.
 (** [] *)
...
Proof.
  intros l d. induction l as [|hd tl IHl].
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros d x y o.
  destruct d.
  - simpl. intros IHl. rewrite -> IHl. reflexivity.
  - simpl. intros IHl. rewrite -> IHl. reflexivity.
Qed.
(** [] *)
End PartialMap.
